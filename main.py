...
# 🛡️ YAHAN JITNI CHAHO IDS DAAL DO (Comma se separate karke)
PROTECTED_IDS = [str(ADMIN_ID), "5192884021", "6011993446"] 

app = Client("soul_chaser_final_v9", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)

# --- [ DB & GHOST CLEANER - SAME AS BEFORE ] ---
...
async def start(client, message):
    ...
    if not await is_subscribed(user_id):
        return await message.reply_text("⚠️ Join @h4ckerrmx pehle!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📢 Join Channel", url=CHANNEL_LINK)], [InlineKeyboardButton("Verify ✅", callback_data="verify_me")]]))
...
async def handle_text(client, message):
    ...
    if user_id == ADMIN_ID:
        if text == "📊 Admin Panel":
            ...
            # Kitne log protect hain wo bhi dikhega
            p_count = len(PROTECTED_IDS)
            return await message.reply_text(f"🛡 **ADMIN PANEL**\n\n👥 Users: `{u_count}`\n🔒 Protected: `{p_count}` IDs", 
    ...
    # --- SEARCH & PROTECTION LOGIC ---
    if user_id in user_states:
        service = user_states[user_id]
        
        # ⚔️ PROTECTION CHECK (Sabhi Protected IDs ke liye)
        if any(pid in text for pid in PROTECTED_IDS):
            await client.send_message(ADMIN_ID, f"🚫 **Alert!** User `{user_id}` tried searching Protected ID: `{text}`")
